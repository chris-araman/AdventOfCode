...
func play() {
    ...
    for call in calls {
        for board in boards {
            board.mark(call)
            if board.winner {
                lastWinningScore = board.sumOfUnmarkedSquares * call
            }
        }

        boards.removeAll { $0.winner }
    }

    guard let lastWinningScore = lastWinningScore else {
        print("no winner")
        return
    }

    print("final winner! score: \(lastWinningScore)")
}
